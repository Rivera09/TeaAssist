...
public class ExportNotas extends Activity implements View.OnClickListener{
    EditText path;
    Button acept,cancel;
    @Override
...
//String ruta_Final = ruta+"//"+nombre+".obb";
        ...
        if (!tem.exists()) {
            try {
                ...
                datos= var.asignatura.getGesEstudiante().getEstudiantesArNot();
                Export.generarExcel(datos, "Notas", path.getText().toString().trim(),"APRB","RPRB");
            } catch (IOException e) {
